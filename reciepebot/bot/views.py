...
from .models import UserSession, OrderItem
...
def whatsapp_reply(request):
    if request.method == 'POST':
        ...
        if incoming_msg.lower() == 'hi':
            ...
            try:
                last_order = UserSession.objects.filter(from_number=from_number, state='completed').latest('id')
                file_path = f"order_{last_order.id}.pdf"
                media_url = request.build_absolute_uri(os.path.join(settings.MEDIA_URL, file_path))
            ...
            try:
                ...
                # Create an OrderItem with quantity only
            ...
            if incoming_msg.lower() == 'yes':
                ...
                session.delete()  # Delete the session after generating the PDF
